...
from swiss_locator.swissgeodownloader.api.datageoadmin import API_EPSG, ApiDataGeoAdmin
...
from swiss_locator.swissgeodownloader.ui.waiting_spinner_widget import QtWaitingSpinner
...
class SwissGeoDownloaderDockWidget(QgsDockWidget, FORM_CLASS):
    ...
    def __init__(self, interface: QgisInterface, locale, parent=None):
        ...
        self.guiExtentWidget.setCurrentExtent(self.canvas.extent(), self.mapRefSys)
        self.guiExtentWidget.setOutputExtentFromCurrent()

        # Initialize class to draw bbox of files in map
        self.bboxPainter = BboxPainter(
            self.canvas, self.transformApi2Proj, self.annManager
        )
        ...
        self.guiRefreshDatasetsBtn.clicked.connect(self.onRefreshCollectionsClicked)
    ...
    def setCurrentCollection(self, collectionId: str):
        ...
        if self.collectionsRequest in QgsApplication.taskManager().activeTasks():
            self.collectionsRequest.taskCompleted.connect(searchAndSelectCollection)
        else:
    ...
    def loadCollectionList(self):
        ...
        self.collectionsRequest = GetCollectionsTask(
            self.apiDGA, self.msgBar, "get STAC collections"
        )
        # Listen for finished api call
        self.collectionsRequest.taskCompleted.connect(
            lambda: self.onReceiveCollections(self.collectionsRequest.output)
        )
    ...
    def onExtentChanged(self):
        ...
        if self.guiExtentWidget.extentState() in [2, 3]:
            ...
            if (
                self.guiExtentWidget.extentState() == 3
                and extentCrs != self.mapRefSys
                and extentCrs.isValid()
            ):
    ...
    def onInfoClicked(self):
        self.showDialog(
            self.tr("Swiss Geo Downloader - Info"),
    ...
    def onReceiveCollections(self, collectionList: dict[str, SgdStacCollection]):
    ...
    def onCollectionSelectionChange(self, collectionId: str):
        ...
        if not self.currentCollection.analysed():
            ...
            caller.taskTerminated.connect(lambda: self.onLoadCollectionDetails())
    ...
    def getBbox(self) -> list:
        ...
        necessary"""
        if self.guiFullExtentChbox.isChecked() or not self.guiExtentWidget.isEnabled():
            return []
    ...
    def onLoadFileListClicked(self):
        ...
        self.fileListRequest = GetFileListTask(
            self.apiDGA,
            self.msgBar,
            "get file list",
        ...
        self.fileListRequest.taskTerminated.connect(lambda: self.onReceiveFileList({}))
    ...
    def getCurrentlySelectedFilesAsList(self) -> list[SgdAsset]:
        return [file for file in self.fileListFiltered.values() if file.selected]
    ...
    def applyFilters(self, userChange=False):
        ...
        #  filter at the time
        if not userChange and len(self.fileList) > 0 and len(orderedFilesForTbl) == 0:
            self.resetFilter()
        ...
        if self.fileListFiltered:
            ...
            for file in self.getCurrentlySelectedFilesAsList():
                ...
                if file.mediaType() in self.currentCollection.avgSize().keys():
                    fileSize += self.currentCollection.avgSize()[file.mediaType()]

            if fileSize > 0:
                status = self.tr("{} file(s), approximately {}").format(
                    count, filesizeFormatter(fileSize)
                )
            else:
    ...
    def onDownloadFilesClicked(self):
        ...
        if len(self.filesListStreamed) == len(filesToDownload):
            # Start spinner to indicate data loading
            createQgisLayersInTask(
                self.filesListStreamed, self.onCreateQgisLayersFinished
            )
    ...
    def startDownload(self):
        ...
        # Listen for finished api call
        caller.taskCompleted.connect(lambda: self.onDownloadFinished(caller.output))
    ...
    def onCreateQgisLayersFinished(
        ...
        if alreadyAdded > 0:
            msg = self.tr(
                "{} layers added to QGIS, {} skipped because they are already present"
            ).format(len(layers), alreadyAdded)

            self.msgBar.pushMessage(
                f"{MESSAGE_CATEGORY}: {msg}", Qgis.MessageLevel.Info
            )
    ...
    def showDialog(title, msg, mode="OkCancel"):
        ...
        return (
            ...
            or returnValue == QMessageBox.StandardButton.Yes
        )
