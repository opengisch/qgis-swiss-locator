...
class TestFileObject(unittest.TestCase):
    ...
    def test_bbox_key_empty(self):
        file_obj = self.createAsset("test_file", "image", "http://example.com")
        self.assertEqual(file_obj.bboxKey, "")

    def test_bbox_key_valid(self):
        file_obj = self.createAsset("test_file", "image", "http://example.com")
        file_obj.setBbox(
            self.createQgsBox3d([100.123456, 10.654321, 102.987654, 12.123456])
        )
    ...
    def test_isStreamable(self):
        file_obj = self.createAsset("myTestFile", "tiff", "https://this.is.a.test.url")
    ...
    def test_timestamp_range_parsing(self):
        ...
        self.assertRaises(
            ValueError, file_obj.setTimestamp, "2023-07-16T12:34:56Z", "this is a test"
        )
